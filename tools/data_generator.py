...
class ImageDataGenerator:
    def __init__(self, batch_size, height, width, z_dim, shuffle=True,
        ...
        #This code check whether the generated image is trained image or test image
        if mode == 'train':
            self.file_folder = '/new_disk2/wangzw/tangxu/age_data/train_data/'
            #It will store the images in 5 groups like 10-20,21-30.......etc
        ...
        self.true_labels = []
        self.false_labels = []
        self.images = [] #This list contain the path of images generated in train/test age_group.txt
        self.labels = []
        self.data_size = []#store the number of images in each class
        self.n_classes = classes
        self.shuffle = shuffle
        self.scale_size = scale_size
        self.label_pair_index = 0 #An index used to keep track of the current label pair index for some kind of comparison.

        self.mean = np.array([104., 117., 124.]) #A numpy array representing the mean pixel values to be subtracted during image preprocessing.
    ...
    def get_age_labels(self):
        ...
        self.age_label = []
        self.age_label.append(np.zeros(batch_size, np.int32)) #This is for age group 0
        self.age_label.append(np.ones(batch_size, np.int32)) #This is for age group 1
        self.age_label.append(np.ones(batch_size, np.int32)*2) #This is for age group 2
        self.age_label.append(np.ones(batch_size, np.int32) * 3) #This is for age group 3 
        self.age_label.append(np.ones(batch_size, np.int32) * 4) #This is for age group 4

    def pre_generate_labels(self, batch_size, height, width):
        # features and one hot labels for each sample, n_class kinds
        features = [] #This list will store label feature arrays for each class. The shape of these arrays will be (height, width, n_classes)
        one_hot_labels = [] 
        full_1 = np.ones((height, width)) #This is a 2D array filled with ones. It's used to fill in the label feature arrays.
