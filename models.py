...
# model_num: A model identifier.
# Optional parameters with default values: batch_size, decay_steps, gan_loss_weight, fea_loss_weight, age_loss_weight, tv_loss_weight.
        self.sess = sess
    ...
    def face_age_alexnet(self, x, scope_name='alexnet', if_age=False, reuse=False):
        with tf.variable_scope(scope_name, reuse=reuse) as scope:
            ...
            # Strides: 4 (both horizontal and vertical)
            ...
            # 6th Layer: Flatten -> FC (w ReLu) -> Dropout to prevent overfitting
            flattened = tf.reshape(pool5, [-1, 6 * 6 * 256])
            fc6 = fc(flattened, 6 * 6 * 256, 4096, name='fc6')
            self.fc6 = fc6
            dropout6 = dropout(fc6, self.KEEP_PROB)
